...
console.log('--- Testing DEM Missing-Tile & Partial Tile Failure Handling ---');

// Construct a synthetic 2x2 tile ElevationGrid (each tile 256x256 -> 512x512 total)
// Zoom 12, covering a known area
const zoom = 12;
const tileXMin = 1320;
const tileXMax = 1321;
const tileYMin = 2880;
...
console.log(`Sample in missing Tile 1: elevation=${sampleTile1.elevation}, isValid=${sampleTile1.isValid}`);
...
console.log('✓ Missing tile gracefully returned NaN with isValid=false (no -32768m crater!)');

// 3. Black pixel crater guard inside valid tile (Tile 2)
// Sample precisely at the pixel location containing NaN
const n = 2 ** zoom;
...
// Because 3 surrounding pixels are 1800m and 1 pixel is NaN, bilinear fallback uses the valid corners
